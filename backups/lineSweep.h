...
namespace lineSweep {
    ...
    public:
        event(
                ...
                seg(iSeg), segSwap(nullptr), x(std::move(iX)), type(t) {};
    ...
    public:
        bool operator()(const event *a, const event *b) {
            ...
            if (a->x[0] < b->x[0] - DMIN_) { return true; }
            if (a->x[0] > b->x[0] + DMIN_) { return false; }
            return a->seg->m + a->segSwap->m > b->seg->m + b->segSwap->m;
        }
    };
    ...
    using sweepSet = std::set<singleSweep *, sweepSorter>;
